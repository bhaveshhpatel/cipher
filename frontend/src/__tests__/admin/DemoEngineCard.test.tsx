...
describe("DemoEngineCard", () => {
  it("renders title and subtitle", () => {
    render(<DemoEngineCard status={null} isRunning={false} loading={false} error={null} toggle={vi.fn()} />);
  ...
  it("shows RUNNING pill when isRunning=true", () => {
    render(<DemoEngineCard status={null} isRunning={true} loading={false} error={null} toggle={vi.fn()} />);
  ...
  it("shows LOADING pill when loading=true", () => {
    render(<DemoEngineCard status={null} isRunning={false} loading={true} error={null} toggle={vi.fn()} />);
    expect(screen.getByText("LOADING…")).toBeInTheDocument();
  });

  it("renders stats when status provided", () => {
    render(<DemoEngineCard status={MOCK_STATUS} isRunning={true} loading={false} error={null} toggle={vi.fn()} />);
  ...
  it("shows '—' for null last_ticker and started_at", () => {
    ...
    render(<DemoEngineCard status={s} isRunning={false} loading={false} error={null} toggle={vi.fn()} />);
    expect(screen.getAllByText("—").length).toBeGreaterThanOrEqual(2);
  });

  it("renders error banner when error is set", () => {
    render(<DemoEngineCard status={null} isRunning={false} loading={false} error="Network error" toggle={vi.fn()} />);
