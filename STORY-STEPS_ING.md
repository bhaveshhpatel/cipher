...
| **ADMIN-UI-001** | **ING-010 ✅ (gate control plane API must be live), `admin_activity_log` table ✅ (migration 014 applied 2026-05-08)** | **✅ Both blockers cleared 2026-05-08. Ready to implement. Issue [#87](https://github.com/bhaveshhpatel/cipher/issues/87). Can run in parallel with ING-008 and ING-012 — no backend changes required.** |
...
| **ADMIN-UI-001** | **✅ COMPLETE (2026-05-08) — 5-way deliberation (SA · UI · FE · PBE · QA) complete. All decisions recorded in Issue [#87](https://github.com/bhaveshhpatel/cipher/issues/87). No backend changes required — `GET /api/admin/gate-config` and `PATCH /api/admin/gate-config` from PR #85 are the full API surface. `admin_activity_log` table created via migration `014_admin_activity_log.sql` applied to `supabase/migrations/` on 2026-05-08. Ready to implement.** |
...
17. **ADMIN-UI-001 `admin_activity_log` table.** ✅ Migration `supabase/migrations/014_admin_activity_log.sql` applied to main 2026-05-08. Schema: `id uuid PK, created_at timestamptz, admin_email text, action text, detail jsonb, ip_address text`. RLS enabled (service_role bypasses). This was the root cause of the production 500 on `GET /api/admin/activity-log` observed in Railway logs 2026-05-08 17:08 UTC (PGRST205 — table not in schema cache). **Do not implement the activity log read path in ADMIN-UI-001 frontend without confirming this migration has been applied to Supabase via the dashboard SQL editor.**
...
- ✅ `admin_activity_log` table blocker resolved 2026-05-08 — migration `supabase/migrations/014_admin_activity_log.sql` pushed to main. Root cause: `backend/migrations/016_admin_activity_log.sql` existed but was never promoted to `supabase/migrations/`. Production 500 on `GET /api/admin/activity-log` (PGRST205) now unblocked once migration is applied to Supabase via dashboard SQL editor.
- ☐ **Apply `014_admin_activity_log.sql` to Supabase production via SQL editor** — migration is in repo but must be run manually against the live DB. Verify `GET /api/admin/activity-log?limit=20&offset=0` returns 200 after apply.
...
| **ADMIN-UI-001** | **Ingestion Gate Control Panel — per-tier gate editor + exclude_indices toggle on Admin page** | **⏳ Ready to implement — ING-010 ✅ 2026-05-08, `admin_activity_log` migration ✅ 2026-05-08 (`014_admin_activity_log.sql` — apply to Supabase SQL editor to resolve prod 500) — Issue [#87](https://github.com/bhaveshhpatel/cipher/issues/87) — 5-way deliberation complete — can run in parallel with ING-008 / ING-012** | **`admin/ing-gate-control-panel`** |

---

*Created: 2026-05-03 | Last updated: 2026-05-08 (Rule 6 constraint 17 added — `admin_activity_log` migration fix; ING-010 post-merge checklist stamped with migration blocker discovery + resolution; ADMIN-UI-001 dependency chain updated to include `admin_activity_log` table prerequisite; Quick Reference updated) | Sprint: WSJ Ingestion Alignment (P0) | Owner: Dhruv Patel*
