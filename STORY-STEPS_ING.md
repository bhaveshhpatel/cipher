...
17. **ADMIN-UI-001 `admin_activity_log` table.** ✅ Migration `supabase/migrations/014_admin_activity_log.sql` committed to main 2026-05-08. ✅ **Applied to Supabase production 2026-05-08** — root cause of PGRST205 500 on `GET /api/admin/activity-log` (observed Railway logs 17:08 UTC) resolved. Schema: `id uuid PK, created_at timestamptz, admin_email text, action text, detail jsonb, ip_address text`. RLS enabled (service_role bypasses). `GET /api/admin/activity-log?limit=20&offset=0` should now return 200.
...
- ✅ `admin_activity_log` migration committed to repo 2026-05-08 (`supabase/migrations/014_admin_activity_log.sql`).
- ✅ **`014_admin_activity_log.sql` applied to Supabase production 2026-05-08** — PGRST205 500 on `GET /api/admin/activity-log` resolved. Verified via Railway logs.
...
| **ADMIN-UI-001** | **Ingestion Gate Control Panel — per-tier gate editor + exclude_indices toggle on Admin page** | **⏳ Ready to implement — ING-010 ✅ 2026-05-08, `admin_activity_log` migration ✅ applied to Supabase production 2026-05-08 — Issue [#87](https://github.com/bhaveshhpatel/cipher/issues/87) — 5-way deliberation complete — can run in parallel with ING-008 / ING-012** | **`admin/ing-gate-control-panel`** |

---

*Created: 2026-05-03 | Last updated: 2026-05-08 (Rule 6 constraint 17 updated — `admin_activity_log` migration ✅ applied to Supabase production; ING-010 post-merge checklist migration checkbox stamped ✅; deliberation table ADMIN-UI-001 updated to reflect applied migration; Quick Reference ADMIN-UI-001 status updated) | Sprint: WSJ Ingestion Alignment (P0) | Owner: Dhruv Patel*
