...
# ============================================================
# RepetitionAccumulator.ingest  (now async -- use asyncio.run)
# ============================================================
...
def test_ingest_returns_none_below_min_premium():
    ...
    result = None
    for i in range(3):
        result = asyncio.run(acc.ingest(_mock_event(premium=100_000, timestamp=_ts(i * 10))))
    assert result is None
...
def test_ingest_returns_episode_when_thresholds_met():
    acc = RepetitionAccumulator(min_trades=3, min_premium=50_000)
    for i in range(2):
        asyncio.run(acc.ingest(_mock_event(premium=100_000, timestamp=_ts(i * 10))))
    ep = asyncio.run(acc.ingest(_mock_event(premium=100_000, timestamp=_ts(20))))
...
def test_ingest_prunes_stale_events():
    ...
    for i in range(2):
        asyncio.run(acc.ingest(_mock_event(premium=100_000, timestamp=_ts(-200 + i))))
    result = asyncio.run(acc.ingest(_mock_event(premium=100_000, timestamp=_ts(0))))
    assert result is None
...
def test_ingest_different_contracts_independent():
    acc = RepetitionAccumulator(min_trades=3, min_premium=50_000)
    for i in range(3):
        asyncio.run(acc.ingest(_mock_event(ticker="AAPL", contract_type="CALL",
                               strike=180.0, premium=100_000, timestamp=_ts(i * 5))))
    result_tsla = asyncio.run(acc.ingest(_mock_event(ticker="TSLA", contract_type="CALL",
                                          strike=250.0, premium=100_000, timestamp=_ts(0))))
    assert result_tsla is None
...
def test_ingest_accumulates_across_calls():
    ...
    for i in range(4):
        result = asyncio.run(acc.ingest(_mock_event(premium=100_000, timestamp=_ts(i * 10))))
        assert result is None
    ep = asyncio.run(acc.ingest(_mock_event(premium=100_000, timestamp=_ts(40))))
    assert ep is not None
    assert ep.trade_count == 5
...
def test_ingest_returns_episode_on_every_qualifying_call():
    ...
    acc = RepetitionAccumulator(min_trades=3, min_premium=50_000)
    for i in range(3):
        asyncio.run(acc.ingest_tick(_mock_event(premium=100_000, timestamp=_ts(i * 10))))
    ep4 = asyncio.run(acc.ingest_tick(_mock_event(premium=100_000, timestamp=_ts(30))))
    ep5 = asyncio.run(acc.ingest_tick(_mock_event(premium=100_000, timestamp=_ts(40))))
