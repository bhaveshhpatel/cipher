...
--
-- Adds a CHECK constraint to both symbol registry tables so that no index
...
--
-- Safe to re-run: IF NOT EXISTS guards prevent duplicate constraint errors.
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    WHERE conname = 'chk_tracked_symbols_no_index'
      AND conrelid = 'tracked_symbols'::regclass
  ) THEN
    ALTER TABLE tracked_symbols
      ADD CONSTRAINT chk_tracked_symbols_no_index
      CHECK (
        UPPER(symbol) NOT IN (
