...
Fix (LAT-1 2026-05-10): guard composite=None after build_composite().
  build_composite() can legitimately return None (e.g. ticker not in backtest
  universe). The composite_msg block must be guarded: `if composite is not None`.
  Previously the code fell through unconditionally, raising AttributeError on
  composite.ticker etc. when build_composite() returned None.

Fix (SYNTAX-002 2026-05-14): restore full file after truncated write in prior session.
  The previous session's create_or_update_file call wrote only the module docstring
  (208 lines) and omitted the entire implementation, leaving an unterminated
  triple-quoted string literal that caused SyntaxError on import and a Render
  deploy failure. This commit restores the complete file content.

Fix (AWAIT-001 2026-05-14): add missing await on accumulator.get_signal(ev).
  get_signal() is an async method. Calling it without await returned a coroutine
  object instead of executing it. Downstream code then operated on the coroutine
  rather than the RepetitionEpisode, producing:
    RuntimeWarning: coroutine 'RepetitionAccumulator.get_signal' was never awaited
    AttributeError: 'OptionsFlowEvent' object has no attribute 'occ'
  The AttributeError was a red herring — the coroutine object was being passed
  where an episode was expected. Fix: `sig_ep = await accumulator.get_signal(ev)`.
  Effect: every tick that cleared the premium floor was failing signal processing
  silently. flow_store was persisting flow_events but zero signals were being
  evaluated. This fix restores signal evaluation on the hot path.

Fix (AWAIT-002 2026-05-14): remove stale accumulator.to_episode(sig_ep) call.
  to_episode() does not exist on RepetitionAccumulator. ingest_tick() and
  get_signal() both return a RepetitionEpisode directly. The stale call would
  have raised AttributeError on every tick that emitted a signal, meaning
  episode persistence and lookback enqueue would have remained broken even
  after AWAIT-001 was applied. Fix: use sig_ep directly (already a
  RepetitionEpisode from get_signal()).
"""
import asyncio
import logging
import random
...
# ING-012: use influence_tier_int() directly — influence_tier_string() deleted
# ---------------------------------------------------------------------------
def _resolve_min_premium(ticker: str) -> int:
    ...
    Resolution path:
      1. Ask the registry for the ticker's influence tier int directly via
         influence_tier_int() (ING-012: influence_tier_string() was deleted).
    ...
    try:
        ...
        if reg is not None and reg.is_ready():
            try:
                tier_int = reg.influence_tier_int(ticker) or _DEFAULT_TIER_INT
            except Exception:
                tier_int = _DEFAULT_TIER_INT
...
def _resolve_tier_int(ticker: str) -> int:
    """
    Resolve the influence tier int for a ticker via the registry.
    ING-012: uses influence_tier_int() directly (influence_tier_string() deleted).
    Falls back to _DEFAULT_TIER_INT (3) on any error.
    """
    try:
        from services.symbol_registry import get_registry as _get_reg
        reg = _get_reg()
        if reg is not None and reg.is_ready():
            return reg.influence_tier_int(ticker) or _DEFAULT_TIER_INT
    except Exception:
        pass
...
async def _process_trade(raw: dict):
    ...
    ING-010-DEDUP addition: tier_int threaded into flow_dedup.is_duplicate()
      so DedupCache reads dedup_window_ms per tier.
    ...
    AWAIT-001 (2026-05-14): sig_ep = await accumulator.get_signal(ev).
      get_signal() is async. Missing await returned a coroutine object,
      causing RuntimeWarning + AttributeError downstream on every qualifying tick.

    AWAIT-002 (2026-05-14): removed stale accumulator.to_episode(sig_ep) call.
      to_episode() does not exist. get_signal() returns RepetitionEpisode directly.
    """
    ...
    # ING-010-DEDUP / REARCH-010: derive tier_int from pre-parse registry lookup
    # (ev.influence_tier was dropped in migration 024). _resolve_tier_int() calls
    # reg.influence_tier_int(ticker) directly (ING-012: influence_tier_string() deleted).
    _ev_tier_int: int = _resolve_tier_int(_raw_ticker)
    ...
    log.debug(
        ...
        f"| sentiment={ev.sentiment} tier_int={_ev_tier_int} "
        f"| occ={occ_symbol} "
        f"| synthetic_quote={ev.is_synthetic_quote}"
    )

    # AWAIT-001: both ingest_tick and get_signal are async — must be awaited.
    # AWAIT-002: get_signal returns RepetitionEpisode directly; no to_episode() call needed.
    ...
    # Resolve alert_level and direction from sig_ep (RepetitionEpisode).
    # ALERT-LEVEL fix: use accumulator.get_alert_level() — not sig_ep.alert_level directly.
    ...
    # LAT-1: guard composite=None — build_composite() can return None when
    # the ticker is not in the backtest universe.
