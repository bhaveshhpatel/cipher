...
Fix (REGISTRY-SIZE-CALL 2026-05-24): call registry.size() not registry.size in logs.
  All four call sites in the `if registry is not None` branch of
  stream_options_flow() were passing the bound method object `registry.size`
  as a %d format argument instead of the int return value `registry.size()`.
  Python's %-style logging defers substitution to LogRecord.getMessage(), so
  the TypeError surfaced at emit time (in the JSON formatter in main.py) rather
  than at the log.info() call site — making it look like a formatter bug.
  Fixed sites:
    - STREAM-GATE-002 log arg
    - STREAM-GATE-003 log arg
    - inline "Registry ready" log arg
    - _stats["active_symbols"] = registry.size()
    - LIVE mode log arg
  The else-branch (standalone path) already used registry.size() correctly.
"""
import asyncio
import logging
import random
...
async def stream_options_flow(
    ...
    if registry is not None:
        ...
        # REGISTRY-SIZE-CALL: registry.size is a method — must call registry.size()
        log.info(
        ...
        # REGISTRY-SIZE-CALL: registry.size() — call the method, not pass the method object
        log.info(
            "[stream] STREAM-GATE-003: registry ready. size=%d, waited=%.1fs. About to call manage