...
Fix (SYNTAX-002 2026-05-24): close unclosed paren in registry.build() call (else branch).
  Line 793: `occ_count, _ = await registry.build(` was missing the closing `)`.
  Python tokenizer held the paren open across the except clause, raising
  SyntaxError: '(' was never closed at startup — server never reached main().
  Fix: add `)` to close the zero-arg registry.build() call.
"""
import asyncio
import logging
import random
...
async def stream_options_flow(
    ...
    # stream invocation including reconne
