...
STREAM-14 (2026-05-20):
  ...
  mid-iteration inside _log_health(). Fix: snapshot workers = list(self._workers)
  at the top of _log_health(). All iteration uses the stable snapshot.

STREAM-15 (2026-05-20):
  Fix spurious ValueError from task_done() in _consume_queue._run_process().
  task_done() was in finally, firing even when get() never succeeded. Fix:
  call task_done() explicitly after process_fn, wrapped in try/except ValueError.
...
class StreamManager:
    ...
    def _log_health(self):
        ...
        # STREAM-14: snapshot workers list to avoid corruption if _respawn_workers()
        # swaps self._workers while sum() generators are mid-iteration.
        workers = list(self._workers)

        total_ticks  = sum(w._ticks       for w in workers)
        total_errors = sum(w._errors      for w in workers)
        total_recon  = sum(w._reconnects  for w in workers)

        ticks_delta = total_ticks - self._total_ticks_at_last_health
        rate        = ticks_delta / elapsed if elapsed > 0 else 0.0

        active   = sum(1 for w in workers if not getattr(w, "_token_expired", False))
        stalled  = sum(
            1 for w in workers
            if w._last_tick_at and (now_wall - w._last_tick_at) > _STALL_THRESHOLD_S
        )
        never_ticked = sum(1 for w in workers if w._last_tick_at is None)
        queue_depth  = self._queue.qsize() if self._queue else 0
    ...
    async def _consume_queue(self):
        ...
        async def _run_process(raw: dict) -> None:
            async with self._process_sem:
                ...
                # STREAM-15: task_done() moved out of finally — calling it
                # unconditionally in finally fires even when get() never
                # succeeded (queue drained / cancelled mid-flight), raising
                # ValueError: task_done() called too many times.
                # Call it exactly once per confirmed get() instead.
                try:
                    self._queue.task_done()
                except ValueError:
                    pass  # belt-and-suspenders guard against any edge-case drift
