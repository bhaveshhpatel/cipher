...
async def _run_demo_loop(tickers: List[str]) -> None:
    ...
    try:
        while True:
            ...
            envelope = _build_timesale_envelope(ticker, expiry, ctype, strike, fill, size, exch)
            try:
                await bus.publish("raw_tick", envelope)
                await bus.publish("demo_tick", envelope)
            except Exception as pub_err:
                log.warning("[demo_engine] bus publish error: %s", pub_err)
