...
FIX C-3 (2026-04-27): assign_tiers() is now called with require_oi=True
  ...
  the tier gate only after build() has populated oi_by_ticker from
  chain fetches.
...
FIX M-1 (2026-04-28): Replaced is_ready() len-check with a dedicated
  ...
  the first DB-seeded contract appears in the registry.
...
FIX M-3 (2026-04-28): _post_build_upsert is split into two separately
  ...
  upsert_symbol_quotes() is skipped.

FIX T-1 (2026-04-29): get_config and _fetch_thresholds are imported at
  module level (not inside build()) so that tests can patch them via
  'services.symbol_registry.get_config' and
  'services.symbol_registry._fetch_thresholds'.
"""
import asyncio
import logging
...
# T-1: module-level imports so tests can patch these names on this module
try:
    from services.ingestion_config import get_config
except Exception:  # pragma: no cover
    async def get_config():  # type: ignore[misc]
        return {}

try:
    from services.tier_engine import _fetch_thresholds, assign_tiers
except Exception:  # pragma: no cover
    async def _fetch_thresholds():  # type: ignore[misc]
        return {}
    async def assign_tiers(quotes, **kw):  # type: ignore[misc]
        return {}
...
class SymbolRegistry:
    def __init__(
        ...
        # with stale DB data. build() sets it at the very end.
    ...
    async def build(self) -> tuple[int, dict[str, dict]]:
        ...
        Returns tuple[int, dict[str, dict]] - (count, raw_quotes).
        """
        # T-1: use module-level imports (patchable by tests)
        import services.symbol_registry as _self_module
        _get_config = getattr(_self_module, "get_config")
        _fetch_thresh = getattr(_self_module, "_fetch_thresholds")
        _assign_tiers = getattr(_self_module, "assign_tiers")

        from services.symbols_loader import SymbolQuote

        cfg, thresh = await asyncio.gather(_get_config(), _fetch_thresh())
        tier_params      = _build_tier_params(thresh, global_min_oi=cfg.get("REGISTRY_MIN_OI", 0))
        ...
        async with self._build_lock:
            ...
            live_tier_map = await _assign_tiers(
                synthetic_quotes,
                thresholds=thresh,
                require_oi=True,
            )
