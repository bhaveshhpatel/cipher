...
# health probe passes. On timeout the function falls through to an empty
# symbol list and _background_universe_resolve() handles the slow path.
_STARTUP_UNIVERSE_TIMEOUT_S = 8.0
...
async def _resolve_startup_universe_fast() -> tuple[list[str], dict[str, int], Optional[str], bool]:
    """STARTUP-TIMEOUT: DB-only fast path for Step 2a.

    The entire body runs inside asyncio.wait_for() with an 8-second hard
    timeout. If the DB stalls (cold Supabase pool on a Render cold boot),
    the function falls through immediately so yield is never blocked.
    _background_universe_resolve() (Step 6b) handles the slow path after
    the server is already live.
    """

    async def _inner() -> tuple[list[str], dict[str, int], Optional[str], bool]:
