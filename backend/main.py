...
Key architectural fixes:
  ...
  D-001-CALLSITE (main) — correct stream_options_flow call in lifespan():
                        was stream_options_flow(registry, _registry_build_done) which
                        passed registry as the `symbols` positional arg — zero workers
                        ever spawned. Fixed to stream_options_flow(stream_symbols, registry=registry).
...
async def _background_build_and_upsert(
    ...
    try:
        ...
        try:
            ...
            log.error("[build] registry.build() failed: %s