...
│  • Hot path: _process_trade() → parser → accumulator → composite → broadcast   │
...
│  • Clean events pass through to both persistence and signal paths              │
└───────────────────────────────────┬─────────────────────────────────────────────┘
                                    │ deduped event
                    ┌───────────────┴──────────────────┐
                    │                                  │
...
│                            │    │                                                │
│  services/flow_store.py    │    │  Enters Apex Signal Layers below               │
│  • Buffer + flush 500ms    │    │                                                │
...
│  ├── Spread gate: T1 spread > 15% → reject                                     │
│  │                T2/T3 spread > 25% → reject                                  │
...
│  ├── Single trade · trade_type == SWEEP · premium >= 500K                     │
...
│  ├── sector_score            × 0.10  (reserved — ladder/context data needed)  │
...
│  • Ladder output feeds sector_score in L3 composite (future)                   │
...
**Hot path:** Each raw tick is routed through `_process_trade()`, which chains: parser → deduplication → L1 gate → accumulator → composite scorer → broadcast. Direction in the final publish step comes from `sig_ep.dominant_direction`, not a naive contract-type shortcut.
...
**Window split at this layer:** Clean deduplicated events are routed to two independent paths. The persistence path uses a 30-minute window (historical breadth). The signal path uses a 10-minute window (real-time precision). These are not shared.
...
**Retained: spread gate.** Wide spreads indicate thin markets where fill pricing is unreliable. Threshold is tiered: T1 symbols use a 15% spread cap; T2/T3 symbols use 25% because thin-market names routinely have wider quoted spreads.
...
**Sweep bypass:** A single massive sweep (>$500K, trade_count=1) can bypass the `min_sweeps` requirement. When a whale hits the market in a single execution at that size, the repetition threshold adds no information.
...
The composite scorer combines multiple independent signal dimensions into a single score and recommendation. It was overhauled to remove fake backtest influence and to correctly use episode-level context rather than per-tick shortcuts.
...
The ladder detector identifies coordinated multi-strike positioning on the same ticker and expiry. This is a form of cross-contract confirmation that is significantly stronger than any single-strike episode in isolation.
...
**Output:** `LadderSignal` with ticker, expiry, strikes list, and combined total premium. This feeds the reserved `sector_score` input in L3 in a future sprint.
...
|---|---|
| S0 — Swarm cleanup | L3 composite (removes async path) |
| S1 — Threshold reconciliation + emit flush | L2 accumulator |
| S2 — Parser + detector fixes | L2 parser |
| S2.5 — DB migration | L5 persistence schema |
| S3 — Signal gate | Apex L1 |
| S4 — Dual-window accumulator | Apex L2 |
| S5 — Ladder detection | Apex L4 |
| S6 — Composite overhaul + hot path | Apex L3, L5 |
| S7 — Tiered swarm | Apex L6 |
| S8 — Real backtest | Apex L3 |
