...
| 6 | **REARCH-006** — Signal Engine Rewrite | [#107](https://github.com/bhaveshhpatel/cipher/issues/107) | `feat/rearch-006-signal-engine-rewrite` | ✅ Merged to aggregation branch | SA · PBE · QA | REARCH-003, REARCH-004, REARCH-005 |
...
> **REARCH-006 note:** ✅ Merged 2026-05-12 via [PR #131](https://github.com/bhaveshhpatel/cipher/pull/131). Full 5-dimension Steamroom conviction gate shipped in `backend/signals/signal_engine.py`. Core deliverables: `SignalEngine` class, `EpisodeEvalResult` dataclass, `evaluate()` → `EpisodeEvalResult` pipeline, `compute_conviction_score()`, `build_signal_row()`, `_derive_recommendation()` (5-value enum: `FOLLOW_SWEEP / BUY_CALLS / BUY_PUTS / WATCH / NO_ACTION`), `SignalDebounce`, and migration `033_rearch006_recommendation_enum.sql`. Significant deliberation and fix cycle completed prior to merge:
>
> - **Fix 4 — All gates hard; `evaluate()` any-fail → `passed=False`:** All 5 Steamroom dimension gates (D1_PREMIUM, D2_ASK_SIDE, D3_VOL_GT_OI, D4_DTE, D5_REPETITION) are unconditionally enforced — no gate is skipped regardless of config flags. A single failing dimension sets `passed=False` and appends to `failing_dimensions`. Root fix resolving the bulk of E-02 through E-32b assertion failures.
> - **SA-01 — D2 kill-switch auto-confirm:** `_derive_recommendation()` accepts `ask_side_confirmed` as a required 3rd parameter. When `require_ask_side=False` (kill-switch), `ask_side_confirmed` is auto-set to `True` in `build_signal_row()` so a score=5 episode correctly resolves to `FOLLOW_SWEEP`. `WARNING` log emitted when kill-switch is active.
> - **QA-01 — `_derive_recommendation()` enum correctness sweep:** Parametrized test across all 36 combinations of `(score 0–5) × (direction BULLISH/BEARISH/NEUTRAL) × (ask_side_confirmed True/False)` proves no value outside `_VALID_RECOMMENDATIONS` can be emitted — guards the `signal_history.recommendation` DB CHECK constraint.
> - **PBE-01 — `_EpisodeProxy.__getattr__` contract:** Raises `AttributeError` for unknown keys (Python data-model default). Two sentinel exceptions (`weighted_premium → 0.0`, `events → []`) explicitly documented. Misleading `dte` sentinel removed; Gate 4 handles missing `dte` via `dte_bucket` fallback path.
