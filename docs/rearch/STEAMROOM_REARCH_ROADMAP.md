...
| 5 | **REARCH-005** — Signal Config Store | [#106](https://github.com/bhaveshhpatel/cipher/issues/106) | `feat/rearch-005-signal-config-store` | 🟡 In Development | SA · PBE · QA | REARCH-002, REARCH-004 |
...
> **REARCH-005 note:** 🟡 In Development. `signal_config` DB table and `signal_config_store.py` partially implemented. Tier multiplier extension (PBE) in progress: `_TIER_MULT_KEYS` dict, `get_effective_premium_threshold(alert_level_key, notional_tier)` API, and 6 multiplier seed rows (`031_seed_signal_config_tier_multipliers.sql`) are designed and agreed upon but not yet merged. Migrations `029_create_signal_config_table.sql` and `030_seed_signal_config_steamroom_defaults.sql` (10 base knobs) status TBD. Branch: `feat/rearch-005-signal-config-store`. **Not yet merged to `cipher-rearch`.** Unblocks: REARCH-006 (full Signal Engine Rewrite) upon merge.
