...
| 6 | **REARCH-006** — Signal Engine Rewrite | [#107](https://github.com/bhaveshhpatel/cipher/issues/107) | `feat/rearch-006-signal-engine-rewrite` | 🟡 In Development | SA · PBE · QA | REARCH-003, REARCH-004, REARCH-005 |
...
> **REARCH-005 note:** ✅ Merged 2026-05-11 via [PR #130](https://github.com/bhaveshhpatel/cipher/pull/130). `signal_config` DB-backed store fully shipped. `backend/services/signal_config_store.py` implements `SignalConfigStore` with 30s TTL cache, `get()`, `get_all()`, `set()`, and `get_effective_premium_threshold(alert_level_key, notional_tier)` API — the latter reads tier multiplier rows to scale base dollar thresholds per T1/T2/T3. Three migrations applied to Supabase production: `029_create_signal_config_table.sql` (table + RLS + indexes), `030_seed_signal_config_steamroom_defaults.sql` (10 base Steamroom knobs), `031_seed_signal_config_tier_multipliers.sql` (6 tier multiplier rows for GOLDEN/BLOCK/NOTEWORTHY × T2/T3). `test_rearch005_signal_config_store.py` covering T-4 amendment (tier multiplier path) and K-1 amendment (cache invalidation on set) included. All tests passing. **Unblocks: REARCH-006 (Signal Engine Rewrite) — the critical path is now clear.**

> **REARCH-006 note:** 🟡 In Development as of 2026-05-12 on branch `feat/rearch-006-signal-engine-rewrite`. Core `SignalEngine` and `EpisodeEvalResult` shipped in `backend/signals/signal_engine.py` (canonical module path; `services/signal_engine.py` deleted — Fix 1). Significant deliberation and fix cycle completed today:
>
> - **Fix 4 — All gates hard; `evaluate()` any-fail → `passed=False`:** All 5 Steamroom dimension gates (D1_PREMIUM, D2_ASK_SIDE, D3_VOL_GT_OI, D4_DTE, D5_REPETITION) are now unconditionally enforced — no gate is skipped regardless of config flags. A single failing dimension sets `passed=False` and appends to `failing_dimensions`. This was the root fix resolving the bulk of the E-02 through E-32b assertion failures.
> - **SA-01 — D2 kill-switch auto-confirm:** `_derive_recommendation()` now accepts `ask_side_confirmed` as a required 3rd parameter. When `require_ask_side=False` (kill-switch), `ask_side_confirmed` is auto-set to `True` in `build_signal_row()` so a score=5 episode correctly resolves to `FOLLOW_SWEEP` instead of silently downgrading to `WATCH`. A `WARNING` log is emitted when the kill-switch is active.
> - **QA-01 — `_derive_recommendation()` enum correctness sweep:** Parametrized test across all 36 combinations of `(score 0–5) × (direction BULLISH/BEARISH/NEUTRAL) × (ask_side_confirmed True/False)` proves no value outside `_VALID_RECOMMENDATIONS` can be emitted — guards the `signal_history.recommendation` DB CHECK constraint (migration 033).
> - **PBE-01 — `_EpisodeProxy.__getattr__` contract clarified:** Contract decision: raise `AttributeError` for unknown keys (Python data-model default). The two sentinel exceptions (`weighted_premium → 0.0`, `events → []`) are explicitly documented. The misleading `dte` sentinel that implied silent-None behaviour was removed; Gate 4 handles missing `dte` via the `dte_bucket` fallback path.
> - **S-01–S-10 — `evaluate()` smoke tests added** to `test_rearch006_signal_engine.py`: covers `GateResult` shape, all-pass, per-gate failure paths, multiple simultaneous failures, GOLDEN/WATCH alert levels, `steamroom_score` accuracy, `config_snapshot` key presence, and `require_ask_side=False` override behaviour.
> - **Import path fix:** All test files updated to import from `signals.signal_engine` (not `services.signal_engine`).
>
> Outstanding before PR: remaining deliberation items from issue [#107](https://github.com/bhaveshhpatel/cipher/issues/107), full boundary-value pass on E-01 through E-35 suite, migration 032/033 (`signal_history` schema additions) verification.
