...
│                                                                  │
│  ING-009 — Episode upsert/merge (2026-05-06):                    │
│  persist_flow_episode() is no longer insert-only. On every call  │
│  it performs an insert-or-PATCH upsert keyed on contract         │
│  identity + 30-min session window:                               │
│    Merge key: (ticker, direction, contract_type, strike, expiry) │
│    _lookup_open_episode() queries flow_episodes for an open row  │
│    matching the merge key within _EPISODE_MERGE_WINDOW_S (1800s).│
│    Match found  → PATCH: trade_count += 1,                       │
│                           total_premium += new_premium,          │
│                           signal_ts = new_ts                     │
│                   _stats["merged_episodes"] += 1                 │
│    No match     → INSERT (original path)                         │
│                   _stats["created_episodes"] += 1                │
│  Both counters exposed in /health/stream from cold start.        │
│  Merge logic lives entirely in flow_store.py — not in            │
│  tradier_stream.py and not in the accumulator (layer boundary    │
│  preserved). ING-007 get_contract_prior_days() depends on        │
│  flow_episodes being correctly aggregated; ING-009 is its        │
│  hard prerequisite.                                              │
│                                                                  │
...
### flow_episodes
One row per qualifying repetition episode per session window. Written via insert-or-PATCH
upsert (ING-009, 2026-05-06): subsequent qualifying prints for the same contract within
`_EPISODE_MERGE_WINDOW_S` (1800s) PATCH the existing row rather than inserting a new one.
